...
"""
run_block: Convert an encoded FullBlock from the Joker blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../joker/types/full_block.py) in json format
...
$ca_root is the directory containing your current Joker config files
$hash is the header_hash of the [BlockRecord](../joker/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../joker/types/condition_opcodes.py)
...
from joker.full_node.generator import create_generator_args
...
from joker.wallet.cat_wallet.cat_utils import match_cat_puzzle
