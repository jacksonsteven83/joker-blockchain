...
if getattr(sys, "frozen", False):
    name_map = {
        ...
        "joker_seeder": "start_seeder",
        "joker_crawler": "start_crawler",
    }
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Joker will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `joker keys migrate` manually.")
