...
from joker.wallet.coin_selection import select_coins
...
class Wallet:
    ...
    async def create_spend_bundle_relative_joker(self, joker_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        ...
        if joker_amount < 0:
            utxos = await self.select_coins(uint64(abs(joker_amount)), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(joker_amount), "memos": []}
                ]
