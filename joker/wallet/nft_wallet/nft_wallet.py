...
from joker.wallet.nft_wallet import nft_puzzles
from joker.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from joker.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from joker.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from joker.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from joker.wallet.payment import Payment
from joker.wallet.puzzle_drivers import PuzzleInfo, Solver
from joker.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from joker.wallet.trading.offer import OFFER_MOD, NotarizedPayment, Offer
from joker.wallet.transaction_record import TransactionRecord
from joker.wallet.util.compute_memos import compute_memos
from joker.wallet.util.debug_spend_bundle import disassemble
from joker.wallet.util.transaction_type import TransactionType
from joker.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, in_transaction: bool) -> None:
        ...
        full_nodes: Dict[bytes32, WSJokerConnection] = server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    async def create_tandem_xjk_tx(
        self, fee: uint64, announcement_to_assert: Optional[Announcement] = None
    ) -> TransactionRecord:
        joker_coins = await self.standard_wallet.select_coins(fee)
        joker_tx = await self.standard_wallet.generate_signed_transaction(
            uint64(0),
        ...
        assert joker_tx.spend_bundle is not None
        return joker_tx
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, joker_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if joker_tx is not None and joker_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, joker_tx.spend_bundle])
            joker_tx = dataclasses.replace(joker_tx, spend_bundle=None)
        ...
        if joker_tx is not None:
            tx_list.append(joker_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            joker_tx = await self.create_tandem_xjk_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, joker_tx
