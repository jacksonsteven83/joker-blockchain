...
from joker.types.weight_proof import SubEpochData, WeightProof
from joker.util.byte_types import hexstr_to_bytes
from joker.util.chunks import chunks
from joker.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from joker.util.default_root import STANDALONE_ROOT_PATH
from joker.util.ints import uint32, uint64
from joker.util.keychain import Keychain, KeyringIsLocked
...
from joker.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from joker.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from joker.wallet.util.wallet_sync_utils import (
...
class WalletNode:
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSJokerConnection] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.jokercoin.org"]),
    ...
    def on_disconnect(self, peer: WSJokerConnection):
    ...
    async def is_peer_synced(
        self, peer: WSJokerConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSJokerConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSJokerConnection]:
        if self.server is None:
            return None
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSJokerConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSJokerConnection):
        if self.wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSJokerConnection) -> int:
        assert self.wallet_state_manager is not None
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSJokerConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSJokerConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_puzzle_solution(self, peer: WSJokerConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSJokerConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSJokerConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
