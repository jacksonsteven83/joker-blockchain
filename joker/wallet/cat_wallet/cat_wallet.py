...
from joker.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from joker.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            joker_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        joker_tx = dataclasses.replace(joker_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(joker_tx)
    ...
    async def create_tandem_xjk_tx(
        ...
        if fee > amount_to_claim:
            ...
            origin_id = list(joker_coins)[0].name()
            ...
            message = None
            for spend in joker_tx.spend_bundle.coin_spends:
            ...
            joker_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in joker_coins])
            joker_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XJK as part of this
        regular_joker_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_joker_to_claim = payment_amount

        need_joker_transaction = (fee > 0 or regular_joker_to_claim > 0) and (fee - regular_joker_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_joker_transaction:
                    if fee > regular_joker_to_claim:
                        joker_tx, _ = await self.create_tandem_xjk_tx(
                            fee, uint64(regular_joker_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_joker_to_claim > fee:
                        joker_tx, _ = await self.create_tandem_xjk_tx(fee, uint64(regular_joker_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        joker_spend_bundle = SpendBundle([], G2Element())
        if joker_tx is not None and joker_tx.spend_bundle is not None:
            joker_spend_bundle = joker_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if joker_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=joker_tx.confirmed_at_height,
                    created_at_time=joker_tx.created_at_time,
                    to_puzzle_hash=joker_tx.to_puzzle_hash,
                    amount=joker_tx.amount,
                    fee_amount=joker_tx.fee_amount,
                    confirmed=joker_tx.confirmed,
                    sent=joker_tx.sent,
                    spend_bundle=None,
                    additions=joker_tx.additions,
                    removals=joker_tx.removals,
                    wallet_id=joker_tx.wallet_id,
                    sent_to=joker_tx.sent_to,
                    trade_id=joker_tx.trade_id,
                    type=joker_tx.type,
                    name=joker_tx.name,
                    memos=[],
                )
