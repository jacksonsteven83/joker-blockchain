...
from joker.protocols.wallet_protocol import CoinStateUpdate, NewPeakWallet
...
class NewPeakQueue:
    ...
    async def full_node_state_updated(self, coin_state_update: CoinStateUpdate, peer: WSJokerConnection):
        await self._inner_queue.put(NewPeakItem(NewPeakQueueTypes.FULL_NODE_STATE_UPDATED, (coin_state_update, peer)))

    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSJokerConnection):
