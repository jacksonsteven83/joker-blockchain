...
from joker.types.blockchain_format.coin import hash_coin_ids, Coin
...
from joker.util.merkle_set import confirm_not_included_already_hashed, confirm_included_already_hashed, MerkleSet
from joker.wallet.util.peer_request_cache import PeerRequestCache

log = logging.getLogger(__name__)


async def fetch_last_tx_from_peer(height: uint32, peer: WSJokerConnection) -> Optional[HeaderBlock]:
...
async def request_and_validate_removals(
    peer: WSJokerConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_and_validate_additions(
    peer: WSJokerConnection, height: uint32, header_hash: bytes32, puzzle_hash: bytes32, additions_root: bytes32
):
...
async def _fetch_header_blocks_inner(
    all_peers: List[WSJokerConnection],
