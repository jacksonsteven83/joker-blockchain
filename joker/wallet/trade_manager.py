...
from joker.wallet.trading.offer import NotarizedPayment, Offer
...
class TradeManager:
    ...
    By default, standard XJK is supported but to support other types of assets you must implement certain functions on
    ...
    Puzzle Drivers:
      - See joker/wallet/outer_puzzles.py for a full description of how to build these
    ...
    async def cancel_pending_offer_safely(
        ...
        for coin in Offer.from_bytes(trade.offer).get_primary_coins():
            ...
            # This should probably not switch on whether or not we're spending a XJK but it has to for now
    ...
    async def _create_offer_for_ids(
        ...
        try:
            ...
            for id, selected_coins in coins_to_offer.items():
                ...
                # This should probably not switch on whether or not we're spending XJK but it has to for now
                if wallet.type() == WalletType.STANDARD_WALLET:
                    tx = await wallet.generate_signed_transaction(
                        abs(offer_dict[id]),
                        Offer.ph(),
