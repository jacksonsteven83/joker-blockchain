...
from joker.util.bech32m import bech32_decode, bech32_encode, convertbits
from joker.util.ints import uint64
from joker.wallet.outer_puzzles import (
    construct_puzzle,
    create_asset_id,
    match_puzzle,
    solve_puzzle,
...
from joker.wallet.util.puzzle_compression import (
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting XJK
        coins: List[Coin],
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "xjk" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
