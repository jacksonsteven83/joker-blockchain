...
from joker.util.ints import uint64, uint32, uint8, uint128
...
from joker.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
...
class DIDWallet:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            joker_tx = await self.create_tandem_xjk_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if joker_tx is not None and joker_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(joker_tx)
