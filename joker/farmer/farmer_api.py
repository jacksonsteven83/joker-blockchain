...
from joker.protocols.harvester_protocol import (
    PlotSyncDone,
    PlotSyncPathList,
    PlotSyncPlotList,
    PlotSyncStart,
...
class FarmerAPI:
    ...
    @api_request
    @peer_required
    async def plot_sync_start(self, message: PlotSyncStart, peer: ws.WSJokerConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].sync_started(message)

    @api_request
    @peer_required
    async def plot_sync_loaded(self, message: PlotSyncPlotList, peer: ws.WSJokerConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_loaded(message)

    @api_request
    @peer_required
    async def plot_sync_removed(self, message: PlotSyncPathList, peer: ws.WSJokerConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_removed(message)

    @api_request
    @peer_required
    async def plot_sync_invalid(self, message: PlotSyncPathList, peer: ws.WSJokerConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_invalid(message)

    @api_request
    @peer_required
    async def plot_sync_keys_missing(self, message: PlotSyncPathList, peer: ws.WSJokerConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_keys_missing(message)

    @api_request
    @peer_required
    async def plot_sync_duplicates(self, message: PlotSyncPathList, peer: ws.WSJokerConnection):
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_duplicates(message)

    @api_request
    @peer_required
    async def plot_sync_done(self, message: PlotSyncDone, peer: ws.WSJokerConnection):
