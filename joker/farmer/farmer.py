...
from joker.plot_sync.delta import Delta
from joker.plot_sync.receiver import Receiver
from joker.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from joker.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'joker keys generate' or open the UI."
        ...
        if "xjk_target_address" not in self.config:
            self.config = config["farmer"]
        if "xjk_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xjk_target_address" not in self.config or "xjk_target_address" not in self.pool_config:
            log.debug("xjk_target_address missing in the config")
            return False
