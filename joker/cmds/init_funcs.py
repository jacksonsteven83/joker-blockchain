...
from joker.util.db_version import set_db_version
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xjk_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xjk_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xjk_target_address") or all_targets[-2] == config["pool"].get(
                    "xjk_target_address"
                ):
        ...
        if "xjk_target_address" not in config["farmer"]:
            print(
                f"Setting the xjk destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def init(
    ...
    if create_certs is not None:
        ...
        return joker_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def joker_init(
    ...
    *,
    ...
    """
    joker_root = os.environ.get("Joker_ROOT", None)
    if joker_root is not None:
        print(f"Joker_ROOT is set to {joker_root}")
