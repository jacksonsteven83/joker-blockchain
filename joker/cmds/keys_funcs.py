...
from joker.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed, unlocks_keyring
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from joker.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
def search_derive(
    ...
    for sk in private_keys:
        ...
        # Otherwise derive from well-known derivation paths
            ...
            current_path_indices: List[int] = [12381, 18444]
            path_root: str = "m/"
            for i in [12381, 18444]:
            ...
            for account in range(7):
                ...
                if show_progress:
                    ...
                    # e.g. 2/ (example full output: m/12381/18444/2/)
...
def derive_wallet_address(
    ...
    if prefix is None:
        ...
        selected: str = config["selected_network"]
        prefix = config["network_overrides"]["config"][selected]["address_prefix"]
    path_indices: List[int] = [12381, 18444, 2]
...
def derive_child_key(
    ...
    if key_type is not None:
        path_indices: List[int] = [12381, 18444]
        path_indices.append(
            {
                "farmer": 0,
                "pool": 1,
