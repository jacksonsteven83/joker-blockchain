...
from joker.cmds.show import print_connections
...
from joker.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
...
from joker.cmds.cmds_util import transaction_submitted_msg, transaction_status_msg
...
def print_transaction(tx: TransactionRecord, verbose: bool, name, address_prefix: str, mojo_per_unit: int) -> None:
    if verbose:
        ...
        joker_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {joker_amount} {name}")
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["joker"])
        ...
        for item in [*offers, *requests]:
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use joker wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_offer_summary(cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False):
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["joker"]
        wid: str = "1" if asset_id == "xjk" else ""
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['joker']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    print("  REQUESTED:")
    await print_offer_summary(cat_name_resolver, requested)
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['joker']}")
    ...
    if not examine_only:
        ...
        if confirmation in ["y", "yes"]:
            ...
            print(f"Use joker wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if confirmation in ["y", "yes"]:
        ...
        if secure:
            print(f"Use joker wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
