...
from joker.cmds.init_funcs import init
from joker.consensus.constants import ConsensusConstants
from joker.daemon.server import WebSocketServer, daemon_launch_lock_path, singleton
...
async def setup_farmer(
    ...
    config["xjk_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xjk")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config["rpc_port"] = uint16(0)
    config_pool["xjk_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xjk")
