...
from joker.ssl.create_ssl import generate_ca_signed_cert, get_joker_ca_crt_key, make_ca_cert
...
def gen_ssl(suffix: str = "") -> None:
    ...
    patch = MonkeyPatch()
    patch.setattr("joker.ssl.create_ssl.write_ssl_cert_and_key", patched_write_ssl_cert_and_key)
    ...
    for node_name, cert_type_dict in node_certs_and_keys.items():
        for cert_type, cert_dict in cert_type_dict.items():
            crt = cert_dict["crt"]
            key = cert_dict["key"]
            ca_crt = joker_ca_crt if cert_type == "public" else private_ca_crt
            ca_key = joker_ca_key if cert_type == "public" else private_ca_key
