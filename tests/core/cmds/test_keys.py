...
from joker.cmds.joker import cli
from joker.cmds.keys import delete_all_cmd, generate_and_print_cmd, show_cmd, sign_cmd, verify_cmd
from joker.util.config import load_config
from joker.util.file_keyring import FileKeyring
from joker.util.keychain import DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
from joker.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper, LegacyKeyring
...
def empty_keyring():
    with TempKeyring(user="user-joker-1.8", service="joker-user-joker-1.8") as keychain:
        yield keychain
...
class TestKeysCommands:
    def test_generate_with_new_config(self, tmp_path, empty_keyring):
        ...
        the correct xjk_target_address entries.
        """
        ...
        # Verify that the config has the correct xjk_target_address entries
        address_matches = re.findall(r"xjk1[^\n]+", result.output)
        assert len(address_matches) > 1
        address = address_matches[0]

        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xjk_target_address"] == address
        assert config["pool"]["xjk_target_address"] == address
    ...
    def test_generate_with_existing_config(self, tmp_path, empty_keyring):
        ...
        the original xjk_target_address entries.
        """
        ...
        assert existing_config["farmer"]["xjk_target_address"] == address
        assert existing_config["pool"]["xjk_target_address"] == address
        ...
        # Verify that the config's xjk_target_address entries have not changed
        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xjk_target_address"] == existing_config["farmer"]["xjk_target_address"]
        assert config["pool"]["xjk_target_address"] == existing_config["pool"]["xjk_target_address"]

    def test_show(self, keyring_with_one_key):
        """
        Test that the `joker keys show` command shows the correct key.
        """
    ...
    def test_show_mnemonic(self, keyring_with_one_key):
        """
        Test that the `joker keys show --show-mnemonic-seed` command shows the key's mnemonic seed.
        """
    ...
    def test_generate_and_print(self):
        """
        Test the `joker keys generate_and_print` command.
        """
    ...
    def test_sign(self, keyring_with_one_key):
        """
        Test the `joker keys sign` command.
        """

        message: str = "hello world"
        hd_path: str = "m/12381/18444/0/1"
        runner = CliRunner()
        result: Result = runner.invoke(
    ...
    def test_sign_non_observer(self, keyring_with_one_key):
        """
        Test the `joker keys sign` command with a non-observer key.
        """
    ...
    def test_verify(self):
        """
        Test the `joker keys verify` command.
        """
    ...
    def test_derive_search(self, tmp_path, keyring_with_one_key):
        """
        Test the `joker keys derive search` command, searching a public and private key
        """
        ...
        assert (
            result.output.find(
                (
                    ...
                    "972941e402d737c13df76fdf43597f7b3c2fa9ed27a (HD path: m/12381/18444/2/9)"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    "Found private key: "
                    "028e33fa3f8caa3102c028f3bff6b6680e528d9a0c543c479ef0b0339060ef36 (HD path: m/12381/18444/2/9)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `joker keys derive search` command, searching for a wallet address
        """
        ...
        result: Result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "xjk1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd",
            ],
        )
        ...
        assert (
            result.output.find(
                (
                    ...
                    "xjk1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd (HD path: m/12381/18444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `joker keys derive search` command, searching for a testnet wallet address
        """
        ...
        result: Result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "txjk1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs2v6lg7",
                "--prefix",
                "txjk",
            ],
        )
        ...
        assert (
            result.output.find(
                (
                    "Found wallet address: "
                    "txjk1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs2v6lg7 (HD path: m/12381/18444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_failure(self, tmp_path, keyring_with_one_key):
        """
        Test the `joker keys derive search` command with a failing search.
        """
    ...
    def test_derive_search_hd_path(self, tmp_path, empty_keyring, mnemonic_seed_file):
        """
        Test the `joker keys derive search` command, searching under a provided HD path.
        """
    ...
    def test_derive_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `joker keys derive wallet-address` command, generating a couple of wallet addresses.
        """
        ...
        assert (
            result.output.find(
                (
                    ...
                    "xjk1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfs6wrfdq"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    ...
                    "xjk1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8sumvfmt"
                )
            )
            != -1
        )
    ...
    def test_derive_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `joker keys derive wallet-address` command, generating a couple of testnet wallet addresses.
        """
        ...
        assert (
            result.output.find(
                (
                    "Wallet address 50 (m/12381n/8444n/2n/50n): "
                    "txjk1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfshfylvn"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    "Wallet address 51 (m/12381n/8444n/2n/51n): "
                    "txjk1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8s3utl6c"
                )
            )
            != -1
        )
    ...
    def test_derive_child_keys(self, tmp_path, keyring_with_one_key):
        """
        Test the `joker keys derive child-keys` command, generating a couple of derived keys.
        """
    ...
    def test_migration_not_needed(self, tmp_path, setup_keyringwrapper, monkeypatch):
        """
        Test the `joker keys migrate` command when no migration is necessary
        """
    ...
    def test_migration_full(self, tmp_path, setup_legacy_keyringwrapper):
        """
        Test the `joker keys migrate` command when a full migration is needed
        """
        ...
        current_version_str = pkg_resources.get_distribution("joker-blockchain").version
    ...
    def test_migration_incremental(self, tmp_path, keyring_with_one_key, monkeypatch):
        ...
        from joker.util import keyring_wrapper
    ...
    def test_migration_checked(self, tmp_path, monkeypatch):
        ...
        assert pkg_resources.get_distribution("joker-blockchain").version == "1.2.11.dev123"
