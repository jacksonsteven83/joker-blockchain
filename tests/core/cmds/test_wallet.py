...
from joker.cmds.wallet_funcs import print_offer_summary
...
@pytest.mark.asyncio
async def test_print_offer_summary_xjk(capsys: Any) -> None:
    summary_dict = {"xjk": 1_000_000_000_000}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "XJK (Wallet ID: 1): 1.0 (1000000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xjk_and_cats(capsys: Any) -> None:
    summary_dict = {
        "xjk": 2_500_000_000_000,
    ...
    assert "XJK (Wallet ID: 1): 2.5 (2500000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xjk_and_cats_with_zero_values(capsys: Any) -> None:
    summary_dict = {
        "xjk": 0,
    ...
    assert "XJK (Wallet ID: 1): 0.0 (0 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xjk_with_one_mojo(capsys: Any) -> None:
    summary_dict = {"xjk": 1}
    ...
    assert "XJK (Wallet ID: 1): 1e-12 (1 mojo)" in captured.out
