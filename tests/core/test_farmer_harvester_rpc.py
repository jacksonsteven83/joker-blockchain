...
from joker.plot_sync.receiver import Receiver
...
from joker.rpc.farmer_rpc_api import (
    FarmerRpcApi,
    FilterItem,
...
from joker.util.config import load_config, lock_and_load_config, save_config
from joker.util.hash import std_hash
from joker.util.ints import uint8, uint16, uint32, uint64
from joker.util.misc import get_list_or_len
...
async def test_farmer_reward_target_endpoints(bt, harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "xjk"), encode_puzzle_hash(new_ph_2, "xjk"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "xjk"), encode_puzzle_hash(observer_pool, "xjk")
    )
    ...
    assert config["farmer"]["xjk_target_address"] == encode_puzzle_hash(observer_farmer, "xjk")
    assert config["pool"]["xjk_target_address"] == encode_puzzle_hash(observer_pool, "xjk")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "xjk")
