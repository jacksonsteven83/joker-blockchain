...
from joker.simulator.full_node_simulator import FullNodeSimulator
...
from joker.util.config import lock_and_load_config, save_config
...
from joker.wallet.transaction_sorting import SortKey
...
async def test_send_transaction(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(await wallet_2.get_new_puzzlehash(), "txjk")
...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await client.send_transaction(
        "1", uint64(1), encode_puzzle_hash(await wallet.get_new_puzzlehash(), "txjk")
    )  # Create a pending tx
    ...
    await client.send_transaction("1", uint64(1), encode_puzzle_hash(ph_by_addr, "txjk"))
    await client.farm_block(encode_puzzle_hash(ph_by_addr, "txjk"))
    await time_out_assert(10, wallet_is_synced, True, wallet_node, full_node_api)
    tx_for_address = await client.get_transactions("1", to_address=encode_puzzle_hash(ph_by_addr, "txjk"))
...
async def test_offer_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    # Create an offer of 5 joker for one CAT
    ...
    assert summary == {"offered": {"xjk": 5}, "requested": {cat_asset_id.hex(): 1}, "infos": driver_dict, "fees": 1}
...
async def test_nft_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    from joker.wallet.nft_wallet.nft_info import NFT_HRP
    ...
    res = await wallet_1_rpc.mint_nft(
        ...
        ["https://www.jokercoin.org/img/branding/joker-logo.svg"],
    )
    assert res["success"]
...
async def test_key_and_address_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(ph, "txjk")
    tx_amount = uint64(15600000)
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["xjk_target_address"] = encode_puzzle_hash(test_ph, "txjk")
        # set pool to second private key
        sk = await wallet_node.get_key_for_fingerprint(pks[1])
        test_ph = create_puzzlehash_for_pk(master_sk_to_wallet_sk(sk, uint32(0)).get_g1())
        test_config["pool"]["xjk_target_address"] = encode_puzzle_hash(test_ph, "txjk")
