...
from joker.wallet.outer_puzzles import create_asset_id, match_puzzle
...
async def test_nft_offer_sell_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata = Program.to(
        [
            ("u", ["https://www.jokercoin.org/img/branding/joker-logo.svg"]),
    ...
    # maker create offer: NFT for xjk
    ...
    offer_did_nft_for_xjk = {nft_to_offer_asset_id: -1, wallet_maker.id(): xjk_requested}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_did_nft_for_xjk, {}, fee=maker_fee
    )
    ...
    expected_royalty = uint64(xjk_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + xjk_requested + expected_royalty
    expected_taker_balance = funds - taker_fee - xjk_requested - expected_royalty
...
async def test_nft_offer_request_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    offer_dict = {nft_to_request_asset_id: 1, wallet_maker.id(): -xjk_offered}
    ...
    # assert payments and royalties
    expected_royalty = uint64(xjk_offered * royalty_basis_pts / 10000)
    expected_maker_balance = funds - maker_fee - xjk_offered - expected_royalty
    expected_taker_balance = funds - 2 - taker_fee + xjk_offered + expected_royalty
    await time_out_assert(10, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(10, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_did_to_did(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    expected_royalty = uint64(xjk_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + xjk_requested + expected_royalty
    expected_taker_balance = funds - 1 - taker_fee - xjk_requested - expected_royalty
    await time_out_assert(10, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(10, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_nft_for_cat(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    offer_did_nft_for_xjk = {nft_to_offer_asset_id: -1, cat_wallet_maker.id(): cats_requested}
