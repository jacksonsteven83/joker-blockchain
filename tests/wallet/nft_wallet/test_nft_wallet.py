...
async def test_nft_wallet_rpc_creation_and_list(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    tr1 = await api_0.nft_mint_nft(
        {
            ...
            "uris": ["https://www.jokercoin.org/img/branding/joker-logo.svg"],
        }
    )
    ...
    tr2 = await api_0.nft_mint_nft(
        {
            ...
            "uris": ["https://jokerlisp.com/img/logo.svg"],
        }
    )
    ...
    assert len(uris) == 2
    assert "https://jokerlisp.com/img/logo.svg" in uris
...
async def test_nft_wallet_rpc_update_metadata(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    uris = coin["data_uris"]
    assert len(uris) == 1
    assert "https://www.jokercoin.org/img/branding/joker-logo.svg" in uris
...
async def test_nft_with_did_wallet_creation(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    resp = await api_0.nft_mint_nft(
        {
            ...
            "hash": "0xD4584AD463139FA8C0D9F68F4B59F185",
            "uris": ["https://www.jokercoin.org/img/branding/joker-logo.svg"],
            "target_address": encode_puzzle_hash(nft_ph, "txjk"),
        }
    )
    assert resp.get("success")
    sb = resp["spend_bundle"]
    ...
    assert did_nft["data_uris"][0] == "https://www.jokercoin.org/img/branding/joker-logo.svg"
...
async def test_nft_transfer_nft_with_did(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    resp = await api_0.nft_transfer_nft(
        dict(
            wallet_id=nft_wallet_0_id,
            target_address=encode_puzzle_hash(ph1, "xjk"),
    ...
    resp = await api_0.nft_mint_nft(
        {
            ...
            "hash": "0xD4584AD463139FA8C0D9F68F4B59F185",
            "uris": ["https://www.jokercoin.org/img/branding/joker-logo.svg"],
            "mu": ["https://www.jokercoin.org/img/branding/joker-logo.svg"],
        }
    )
    assert resp.get("success")
    sb = resp["spend_bundle"]
