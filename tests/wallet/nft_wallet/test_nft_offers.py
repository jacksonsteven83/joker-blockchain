...
async def test_nft_offer_with_fee(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    # MAKE FIRST TRADE: 1 NFT for 100 xjk
    ...
    xjk_request = 100
    maker_fee = uint64(10)
    offer_nft_for_xjk = {wallet_maker.id(): xjk_request, nft_asset_id: -1}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_xjk, driver_dict, fee=maker_fee
    )
    ...
    await time_out_assert(15, wallet_maker.get_confirmed_balance, maker_balance_pre + xjk_request - maker_fee)
    await time_out_assert(15, wallet_taker.get_confirmed_balance, taker_balance_pre - xjk_request - taker_fee)
    ...
    # MAKE SECOND TRADE: 100 xjk for 1 NFT
    ...
    xjk_offered = 1000
    maker_fee = uint64(10)
    offer_xjk_for_nft = {wallet_maker.id(): -xjk_offered, nft_to_buy_asset_id: 1}
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_xjk_for_nft, driver_dict_to_buy, fee=maker_fee
    )
    await asyncio.sleep(1)
    assert success is True
    assert error is None
    ...
    await time_out_assert(15, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(15, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(15, wallet_maker.get_confirmed_balance, maker_balance_pre - xjk_offered - maker_fee)
    await time_out_assert(15, wallet_taker.get_confirmed_balance, taker_balance_pre + xjk_offered - taker_fee)
...
async def test_nft_offer_nft_for_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata_2 = Program.to(
        [
            ("u", ["https://www.jokercoin.org/image2.html"]),
