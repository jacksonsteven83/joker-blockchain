...
from joker.clvm.spend_sim import SimClient, SpendSim
...
from joker.wallet.nft_wallet.nft_puzzles import (
...
async def test_default_transfer_program(setup_sim: Tuple[SpendSim, SimClient]) -> None:
    ...
    try:
        ...
        xjk_coin = (await sim_client.get_coin_records_by_puzzle_hash(ACS_PH, include_spent_coins=False))[0].coin
        ...
        xjk_announcement_spend = CoinSpend(
            xjk_coin,
            ACS,
        ...
        for i in range(1, 3):
            for announcement_combo in itertools.combinations(
                [did_announcement_spend, xjk_announcement_spend, cat_announcement_spend], i
            ):
        ...
        full_bundle = SpendBundle(
            [ownership_spend, did_announcement_spend, xjk_announcement_spend, cat_announcement_spend], G2Element()
        )
