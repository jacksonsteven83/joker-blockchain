...
from joker.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
...
from joker.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XJK coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            joker_coins: List[Coin] = all_coins[None]
            ...
            # Create an XJK Offer for RED
            joker_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            joker_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                joker_requested_payments, joker_coins
            )
            joker_announcements: List[Announcement] = Offer.calculate_announcements(joker_requested_payments, driver_dict)
            joker_secured_bundle: SpendBundle = generate_secure_bundle(joker_coins, joker_announcements, 1000)
            joker_offer = Offer(joker_requested_payments, joker_secured_bundle, driver_dict)
            assert not joker_offer.is_valid()

            # Create a RED Offer for XJK
            ...
            new_offer = Offer.aggregate([joker_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for XJK and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xjk": 900, str_to_tail_hash("red").hex(): 350},
