...
# Builds CLI only rpm
CLI_RPM_BASE="joker-blockchain-cli-$JOKER_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/joker"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/joker/"
ln -s ../../opt/joker/joker "dist/$CLI_RPM_BASE/usr/bin/joker"
...
fpm -s dir -t rpm \
  ...
  --name joker-blockchain-cli \
  --license Apache-2.0 \
  --version "$JOKER_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Joker is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  --depends /usr/lib64/libcrypt.so.1 \
  .
# CLI only rpm done

cp -r dist/daemon ../joker-blockchain-gui/packages/gui
cd .. || exit
...
--appVersion=$JOKER_INSTALLER_VERSION --executable-name=joker-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	...
	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Joker-Network/joker-blockchain/issues/3846
  ...
  --license ../LICENSE --options.bin joker-blockchain --options.name joker-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
